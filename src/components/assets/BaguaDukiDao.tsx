...
import BaguaSvg from '../../public/bagua.svg';  // If in public/baguadao folder
import { BaguaSections, DualityDaoLoveSection, Section } from '../baguaDao/types';
...
const getPointOnCircle = (section: Section, offset: number = 140) => {
...
export const BaguaDukiDAO: React.FC<BaguaDukiDAOProps> = ({
  ...
  const getDynamicStyles = (section: Section): React.CSSProperties => {
    ...
    if (section.number === DualityDaoLoveSection.number || section.number === internalSection.number) {
      return {
        fill: section.color,
        opacity: section.opacity,
        stroke: '#00f',
        strokeWidth: section.number === DualityDaoLoveSection.number ? 8 : 0,
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="547.41479" height="547.41479"
      ...
      <g id="yinyang_dao_love" onClick={handleClick(DualityDaoLoveSection)} >
        ...
        {BaguaSections.map((section, index) => {
          ...
          const isSelected = internalSection.number === section.number
        ...
        <circle cx="273.70739" cy="273.70739" r="83" fill="#fff" stroke="black" stroke-width="2" />
